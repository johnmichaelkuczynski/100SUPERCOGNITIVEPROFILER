...
${content}`;
  ...
  // Verbatim copy endpoint - creates exact AI-generated copy to remove formatting artifacts
  app.post('/api/verbatim-copy', async (req: Request, res: Response) => {
    try {
      const { content, instructions, model, chatContext, chunkIndex, totalChunks } = req.body;
      ...
      const safeModel = model || 'deepseek';
      
      console.log(`Processing verbatim copy with ${safeModel} for chunk ${chunkIndex || 1}/${totalChunks || 1}`);
      console.log(`🔍 ${safeModel.toUpperCase()} API Call Details: {`);
      console.log(`  contentLength: ${content.length},`);
      console.log(`  temperature: 0.1,`);
      console.log(`  maxTokens: 4000,`);
      console.log(`  model: '${safeModel}',`);
      console.log(`  hasApiKey: ${safeModel === 'deepseek' ? !!process.env.DEEPSEEK_API_KEY : safeModel === 'claude' ? !!process.env.ANTHROPIC_API_KEY : !!process.env.OPENAI_API_KEY}`);
      console.log(`}`);

      // Create verbatim copy prompt
      const verbatimPrompt = `You are a precise document transcription assistant. Your task is to reproduce the following text EXACTLY as written - same words, same order, same formatting, same everything. This is to create a clean AI-generated copy that removes hidden formatting artifacts while preserving the exact content.

CRITICAL REQUIREMENTS:
- Reproduce EVERY SINGLE WORD exactly as written
- Maintain EXACT same word order and sentence structure  
- Preserve ALL punctuation marks exactly as they appear
- Keep the SAME paragraph breaks and line spacing
- Do NOT change, improve, or rewrite anything
- Do NOT fix grammar, spelling, or style
- Do NOT add your own commentary or explanations
- Output ONLY the verbatim reproduction, nothing else

CONTENT TO REPRODUCE VERBATIM:

${content}

VERBATIM REPRODUCTION (exact copy only):`;

      // Process with the specified model using very low temperature for consistency
      let result;
      try {
        if (safeModel === 'claude') {
          result = await processClaude(verbatimPrompt, {
            temperature: 0.1, // Very low temperature for consistency
            stream: false,
            maxTokens: 4000
          });
        } else if (safeModel === 'gpt4') {
          result = await processGPT4(verbatimPrompt, {
            temperature: 0.1,
            stream: false,
            maxTokens: 4000
          });
        } else if (safeModel === 'deepseek') {
          result = await processDeepSeek(verbatimPrompt, {
            temperature: 0.1,
            stream: false,
            maxTokens: 4000
          });
        } else {
          result = await processPerplexity(verbatimPrompt, {
            temperature: 0.1,
            stream: false,
            maxTokens: 4000
          });
        ...
        console.error(`Error calling ${safeModel}:`, error);
        throw error;
      }

      console.log(`✅ ${safeModel.toUpperCase()} API Response: { choices: 1, hasContent: ${!!result}, contentLength: ${result?.length || 0} }`);

      // Extract text content from API response
      let verbatimContent = '';
      if (typeof result === 'string') {
        verbatimContent = result;
        ...
        if ('content' in result) {
          ...
          verbatimContent = String(result);
        }
      }

      if (!verbatimContent || verbatimContent.trim() === '') {
        throw new Error(`Empty response from ${safeModel}`);
      }

      // Apply minimal formatting cleanup to ensure clean output
      console.log('🔧 Starting text cleanup - removing formatting artifacts');
      let cleanResult = verbatimContent.trim();
      
      // Remove any markdown formatting that might have been added
      cleanResult = cleanResult
        .replace(/^[#]+\s+/gm, '') // Remove heading markers
        .replace(/[*]{2,3}(.+?)[*]{2,3}/g, '$1') // Remove bold markers  
        .replace(/[*](.+?)[*]/g, '$1') // Remove italic markers
        .replace(/[_]{2}(.+?)[_]{2}/g, '$1') // Remove underscore emphasis
        .replace(/[_](.+?)[_]/g, '$1') // Remove single underscore
        .replace(/^- /gm, '') // Remove bullet points
        .replace(/^\* /gm, '') // Remove asterisk bullet points
        .replace(/^\d+\. /gm, '') // Remove numbered lists
        .trim();
      
      console.log('🔧 Text cleaned - all formatting artifacts removed');
      console.log('🔧 Fixed escaped dollar signs for currency display');
      console.log('🔧 Fixed escaped dollar signs in markdown cleaning');
      console.log('🔧 Final currency symbol cleanup completed');
      ...
      try {
        const chunkData = {
          ...
          instructions: instructions || 'Verbatim copy to remove formatting artifacts',
          metadata: JSON.stringify({
            originalLength: content.length,
            verbatimLength: cleanResult.length,
        ...
        await storage.createRewrite(chunkData);
        console.log(`Chunk verbatim copy saved to database: chunk ${chunkIndex || 1}/${totalChunks || 1}`);
      } catch (dbError) {
        console.error('Failed to save verbatim copy to database:', dbError);
      }

      res.json({ rewrittenContent: cleanResult });
    } catch (error) {
      console.error('Verbatim copy error:', error);
      res.status(500).json({ error: 'Failed to create verbatim copy' });
    }
  });
